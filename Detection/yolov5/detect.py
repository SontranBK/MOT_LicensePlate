import numpy as np
from yolov5.utils.general import non_max_suppression, scale_coords,increment_path
...
class Yolov5():
    def __init__(self, list_objects=None):  # if
        ...
        self.path_model = os.path.join(path_cur, "weights/yolov5m6.pt")
        self.model = DetectMultiBackend(self.path_model, device=self.device, dnn=False, data=None, fp16=False)
    ...
    def detect(self, im0s, margin=0, vis=False):
        ...
        '''
        w, h = im0s.shape[:2]
        img = letterbox(im0s.copy(), new_shape=self.img_size,stride=self.stride, auto=self.pt)[0]
        ...
        if img.ndimension() == 3:
            img = img.unsqueeze(0)
        visualize=True
        # visualize = increment_path("./test" / Path("./").stem, mkdir=True) if visualize else False
        ...
        confs = []
        cls_ids = []
        pred= non_max_suppression(pred, self.conf_thres, self.iou_thres, classes=None, agnostic=None, max_det=100)
